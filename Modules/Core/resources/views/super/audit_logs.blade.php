...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('super.dashboard') }}" class="px-4 py-2 rounded border text-sm bg-white hover:bg-gray-50">Kembali ke Dashboard</a>
    </div>

    <!-- Filter Card -->
    <div class="bg-white rounded-lg shadow border border-gray-100 p-4">
        <form action="{{ route('super.audit-logs') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">Tenant</label>
                <select name="tenant_id" class="w-full border rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">Tipe Data (Model)</label>
            ...
            <div>
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->anyFilled(['tenant_id', 'user_id', 'event', 'auditable_type', 'date']))
                        <a href="{{ route('super.audit-logs') }}" class="bg-gray-100 text-gray-700 px-3 py-1.5 rounded text-sm hover:bg-gray-250 flex items-center justify-center">Reset</a>
                    @endif
                </div>
            </div>
        </form>
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap font-medium">{{ $log->tenant->name ?? 'System' }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-gray-500">Tidak ada log aktivitas yang tercatat.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
